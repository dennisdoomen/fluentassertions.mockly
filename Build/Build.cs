...
class Build : NukeBuild
{
    ...
    Target RunInspectCode => _ => _
        ...
        .Executes(() =>
        {
            InspectCode($"{Solution.Path} -o={ArtifactsDirectory / "CodeIssues.sarif"} --no-build");
        });
    ...
    Target RunTests => _ => _
        ...
        .Executes(() =>
        {
            ...
            var specProjects = new[]
            {
                Solution.GetProject("FluentAssertions.Mockly.v7.Specs"),
                Solution.GetProject("FluentAssertions.Mockly.v8.Specs")
            };

            foreach (var project in specProjects)
            {
                DotNetTest(s => s
    ...
    Target ApiChecks => _ => _
        ...
        .Executes(() =>
        {
            var project = Solution.GetProject("FluentAssertions.Mockly.ApiVerificationTests");
    ...
    Target ScanPackages => _ => _
        .DependsOn(Compile)
        // PackageGuard may call GitHub's API and requires a valid token; skip locally to keep dev builds reliable.
        .OnlyWhenStatic(() => !IsLocalBuild && !string.IsNullOrWhiteSpace(GitHubApiKey))
        .Executes(() =>
        {
    ...
    Target Push => _ => _
        ...
        .Executes(() =>
        {
            var packages = ArtifactsDirectory.GlobFiles("FluentAssertions.Mockly.*.nupkg");
